-- Apply OTP Password Reset Migration
...
-- Create policies
DROP POLICY IF EXISTS "Users can view their own OTP records" ON public.otp_password_reset;
CREATE POLICY "Users can view their own OTP records" ON public.otp_password_reset
    FOR SELECT USING (auth.uid() = user_id OR auth.uid() IS NULL);

DROP POLICY IF EXISTS "Allow OTP creation" ON public.otp_password_reset;
CREATE POLICY "Allow OTP creation" ON public.otp_password_reset
    FOR INSERT WITH CHECK (true);

DROP POLICY IF EXISTS "Allow OTP updates" ON public.otp_password_reset;
