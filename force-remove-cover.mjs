...
async function forceRemoveCover() {
  console.log('🔧 FORCE REMOVING COVER IMAGE FROM "NOTRE MISSION" POST...');
  ...
  try {
    ...
    // Force remove the cover image by setting it to null
    const { data: updateData, error: updateError } = await supabase
      .from('posts')
      .update({
        image: null,  // ← Force to null
    ...
    console.log('✅ COVER IMAGE FORCE REMOVED!');
    ...
    if (updatedPost.image === null) {
      console.log('✅ SUCCESS: Cover image is now NULL');
    } else {
      console.log('❌ FAILED: Cover image is still present');
    }
